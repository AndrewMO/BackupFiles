...
if __name__ == '__main__':
    ...
    # cmd = ['cat /proc/version']  # 你要执行的命令列表


    cmd = ['cat /opt/active/sites/acm01vegasjetty/ActiveNetServlet/config/ignite-cache.xml | grep copy']  # 你要执行的命令列表

    cmd1 = ['cat /opt/active/sites/ignite01/ActiveNetServlet/config/ignite-cache.xml | grep copy']  # 你要执行的命令列表


    # cmd = ['cat /opt/active/sites/acm01vegasjetty/ActiveNetServlet/config/service.properties | grep PST']  # 你要执行的命令列表
